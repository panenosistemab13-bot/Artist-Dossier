  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto relative z-10 w-full flex-1 flex flex-col">
        <button 
            id="create-artist-btn"
            onClick={() => setIsCreating(true)}
            className="fixed top-6 right-6 flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-widest text-white bg-white/10 backdrop-blur-sm border border-white/10 rounded-full hover:bg-white/20 transition-all group z-50 shadow-sm cursor-pointer"
        >
          <Plus className="w-3.5 h-3.5 group-hover:rotate-90 transition-transform duration-300 text-amber-500" />
          Novo Artista
        </button>
        ...
        {isCreating ? (
          ...
          <div className="flex flex-col items-center gap-12 pt-4 md:pt-16 max-w-4xl mx-auto w-full text-center">
            {/* Context Header */}
            <div className="w-full text-center flex flex-col items-center max-w-2xl px-4">
              ...
              <h1 className="text-7xl sm:text-8xl md:text-9xl font-serif text-white leading-[0.9] tracking-tighter mb-6 md:mb-8 drop-shadow-md text-center">
                Artist<br />
                <span className="text-stone-300">Dossier.</span>
              </h1>
              
              <p className="text-stone-300 text-lg md:text-2xl font-medium leading-relaxed mb-8 md:mb-12 max-w-2xl mx-auto drop-shadow-sm text-center">
                Gerencie carreiras, organize catálogos de lançamentos e controle distribuições musicais em um ecossistema inteligente e integrado.
              </p>
            </div>
        ...
        <footer className="mt-auto pt-16 pb-4 text-center text-xs text-stone-500 font-medium tracking-widest uppercase">
          Criado por Jefferson Augusto <br /> @jeff_diss
        </footer>
      </div>
    </div>
  );
...
const ArtistListItem: React.FC<ArtistListItemProps> = ({
  ...
  return (
    <div 
      ...
      className="group bg-black/40 backdrop-blur-md rounded-2xl p-2 shadow-lg hover:shadow-xl hover:shadow-stone-900/50 transition-all duration-500 cursor-pointer border border-white/10 hover:border-white/20 flex items-center gap-4 relative overflow-hidden w-fit"
      style={{ animationDelay: `${index * 50}ms` }}
    >
      <div className="w-20 h-20 rounded-xl overflow-hidden shrink-0 shadow-inner relative">
        <img 
          src={resolvedImage} 
          alt={artist.name} 
      ...
      <div className="flex flex-col justify-center pr-2 relative z-10">
        <div className="flex items-center gap-2 mb-0.5">
            <h3 className="font-serif text-lg text-white leading-tight tracking-tight group-hover:text-amber-400 transition-colors truncate">{artist.name}</h3>
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="p-1 rounded-full bg-stone-950/40 hover:bg-red-950/40 text-stone-400 hover:text-red-400 transition-all shrink-0"
              title="Remover Artista"
            >
              <Trash2 className="w-3 h-3" />
            </button>
        </div>
        <div className="flex items-center gap-1 text-stone-300 font-medium text-[10px] tracking-wider uppercase">
          <span className="font-bold">Acessar Dossiê</span>
          <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform duration-300 text-amber-500" />
        </div>
      </div>

...
export function ArtistDossier({ artist, onBack }: ArtistDossierProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F5] text-stone-900 font-sans flex flex-col antialiased selection:bg-amber-200">
      {/* Dynamic Top Bar */}
      <div className="w-full bg-white/50 border-b border-stone-200 backdrop-blur-sm sticky top-0 z-30 px-6 py-3 flex items-center justify-between">
        <button 
          onClick={onBack}
          className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-stone-500 hover:text-stone-900 transition-colors"
        >
